...
El bot guarda todo en una base de datos en Cloudflare (D1). Tú la consultas,
...
> **El nombre de tu base sale de `wrangler.toml`, no es fijo.** Cada bot lleva el suyo
> (`juancitoads_<giro>_<uid>_db`) para que dos clientes nunca compartan datos. Léelo primero
> y úsalo en lugar de `<DB>` en todos los comandos de abajo:
>
> ```bash
